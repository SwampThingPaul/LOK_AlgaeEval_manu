...
Source code and essential data to support Julian et al. (*Submitted*) Planning for the future, algae bloom dynamics in water management and ecosystem restoration efforts. 
...
Some files are too large to store on Github, however `dat_proc.R` has the code necessary to download publicly available files or retrieve data from publicly available sources. The following files were saved to a local drive:
  ...
  * `LOK_sonde.csv`
    * Daily average sonde data including phycocyanin sensor data within Lake Okeechobee
  * `RSMBN_LOK.csv`
  ...
  * `LOK_AlgaeEval_analysis.RData`
    * Complete R session results of data analysis created by running `\src\dat_analysis.R`
